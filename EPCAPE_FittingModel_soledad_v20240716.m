%% EPCAPE_FittingModel_soledad_v20240716.m
%
%
...
%
  ...
  %         SIO
  %         July 19,2024
  % Notes: Modified variable name for EPCAPE analysis of Soledad distribution and added section for grpahing
...
open_filename = 'EPCAPE_SEMS_APS_merged_5min_20240716.mat';
open_filename = strcat(data, open_filename);
...
for ii = 1:length(time)
    %we could modify above to run the fitting model to be of certain range
...
%% Saving the outputs from fitting model
save('no_CPCfilter_mode_fit_out_all_n5_SEMS_Fit_0.70_0716.mat',...
    'mode1_N','mode1_diam','mode1_gsd',...
    'mode2_N','mode2_diam','mode2_gsd',...
    'mode3_N','mode3_diam','mode3_gsd',...
    'chi2_mode1','chi2_mode2','chi2_mode3',...
    'time_5min','flag','stored_SEMS_fit_r2')
 %
...
%% Chi squared values
% 
% chi2_crit_mode1 = chi2inv(0.99, length(d_start_mode1:d_end_mode1)-1);
% chi2_crit_mode2 =(chi2inv(0.99, length(d_start_mode2:d_end_mode2) - 1));
% chi2_crit_mode3 =(chi2inv(0.99, length(d_start_mode3:d_end_mode3) - 1));
% 
% %% Percentage passing each modes
% good_mode1=length(find(chi2_mode1 <= chi2_crit_mode1))./length(find(~isnan(chi2_mode1)==1));
% good_mode2=length(find(chi2_mode2 <= chi2_crit_mode2))./length(find(~isnan(chi2_mode2)==1));
% good_mode3=length(find(chi2_mode3 <= chi2_crit_mode3))./length(find(~isnan(chi2_mode3)==1));
...
% for 1:4:length(time_5min) % we can change these ranges
%     idxplots=i:i+3;
%     for j=1:4
%     subplot(2,2,j)
%     figure(1)
...
%     %%used below 2 lines when saving plots
%     printname=strcat('/Users/atsushiosawa/Downloads/EPCAPE_PIER_APSSMPS/check_fitting_',num2str(idxplots(1)),'_',num2str(idxplots(end)));
